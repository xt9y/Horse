    ...
    std::string *error)
{
    ...
    if (!bytes || !cursor || !pixel || *cursor > size || size - *cursor < count) {
    ...
    Image *image)
{
    ...
    const std::size_t x = right_origin ? static_cast<std::size_t>(width) - 1u - source_x : source_x;
    const std::size_t y = top_origin ? source_y : static_cast<std::size_t>(height) - 1u - source_y;
...
{
    if (!data || size < 18u) return false;
    const std::uint8_t color_map_type = data[1];
    const std::uint8_t image_type = data[2];
    const std::uint16_t width = read16(data + 12u);
    const std::uint16_t height = read16(data + 14u);
    const std::uint8_t depth = data[16];
    return color_map_type == 0u &&
        (image_type == 2u || image_type == 10u) &&
        width != 0u && height != 0u &&
        (depth == 24u || depth == 32u);
}
    ...
    std::string *error)
{
    ...
    *image = {};
    if (!data || size < 18u) return fail(error, "truncated TGA header");
    ...
    if (color_map_type != 0u) return fail(error, "color-mapped TGA is not supported");
    ...
    if (cursor > size) return fail(error, "truncated TGA image id");

    Image decoded;
    decoded.width = width;
    decoded.height = height;
    decoded.rgba.assign(pixel_count * 4u, 0u);
    decoded.meaningful_alpha = false;

    std::size_t output = 0u;
    if (image_type == 2u) {
        while (output < pixel_count) {
            Pixel pixel;
            if (!readPixel(data, size, &cursor, depth, &pixel, error)) return false;
            writePixel(output++, width, height, descriptor, pixel, &decoded);
        }
    } else {
        while (output < pixel_count) {
            if (cursor >= size) return fail(error, "truncated TGA RLE packet header");
            ...
            if ((packet & 0x80u) != 0u) {
                Pixel pixel;
                if (!readPixel(data, size, &cursor, depth, &pixel, error)) return false;
                for (std::size_t i = 0u; i < count; ++i) writePixel(output++, width, height, descriptor, pixel, &decoded);
            } else {
...
bool load(const std::string& path, Image *image, std::string *error)
{
    ...
    if (file_size < 0) return fail(error, "failed to size TGA: " + path);
    input.seekg(0, std::ios::beg);
    std::vector<std::uint8_t> bytes(static_cast<std::size_t>(file_size));
    if (!bytes.empty()) input.read(reinterpret_cast<char *>(bytes.data()), static_cast<std::streamsize>(bytes.size()));
    if (!input && !bytes.empty()) return fail(error, "failed to read TGA: " + path);
    return decode(bytes.data(), bytes.size(), image, error);
}
