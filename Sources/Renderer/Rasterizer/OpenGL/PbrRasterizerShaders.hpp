...
vec2 environmentUv(vec3 direction)
{
    ...
    float angle = uEnvironmentRotation;
    float c = cos(angle);
    float s = sin(angle);
...
vec3 environmentColor(vec3 direction, float roughness)
{
    ...
    vec3 sample_color = pow(max(texture2D(uEnvironment, environmentUv(direction)).rgb, vec3(0.0)), vec3(2.2));
    return mix(sample_color, average, clamp(roughness * roughness, 0.0, 1.0)) * uEnvironmentIntensity;
}
...
float spotAttenuation(vec3 light_direction_to_surface)
{
    ...
    float denominator = max(uSpotInnerCos - uSpotOuterCos, 1.0e-5);
    return clamp((cosine - uSpotOuterCos) / denominator, 0.0, 1.0);
}

float fogVisibility(float distance_to_camera)
{
    if (uFogMode == 1) {
        return clamp((uFogEnd - distance_to_camera) / max(uFogEnd - uFogStart, 1.0e-5), 0.0, 1.0);
    }
...
void main()
{
    ...
    vec3 albedo = max(uBaseColor.rgb, vec3(0.0)) *
        pow(max(texel.rgb, vec3(0.0)), vec3(2.2));
    ...
    if (uLightType != 0 && uLightIntensity > 0.0) {
        ...
        if (uLightType == 1 || uLightType == 3) {
            ...
            if (uLightType == 3)
                incoming *= spotAttenuation(vWorldPosition - uLightPosition);
    ...
    vec3 indirect_irradiance = sampleGi(vWorldPosition, normal);
    vec3 diffuse_indirect = albedo * (1.0 - metallic) * ao * indirect_irradiance / PI;
    vec3 ambient = max(uAmbientColor, vec3(0.0)) * max(uAmbientIntensity, 0.0) * albedo * ao;

    vec3 reflected = reflect(-view_direction, normal);
    vec3 env_specular = environmentColor(reflected, roughness);
    vec3 f0 = mix(vec3(0.04), albedo, metallic);
    vec3 env_fresnel = fresnelSchlick(max(dot(normal, view_direction), 0.0), f0);
    vec3 env_diffuse = max(uEnvironmentAverage, vec3(0.0)) * max(uEnvironmentIntensity, 0.0) *
        albedo * (1.0 - metallic) * ao / PI;
    vec3 ibl = env_diffuse + env_specular * env_fresnel * ao;

    vec3 emissive = max(uEmissiveColor, vec3(0.0)) * max(uEmissiveStrength, 0.0);
    if (uHasEmissiveMap != 0)
        emissive *= pow(max(texture2D(uEmissiveMap, vUv).rgb, vec3(0.0)), vec3(2.2));

    vec3 linear_color = max(direct + diffuse_indirect + ambient + ibl + emissive, vec3(0.0));
    float fog_visibility = fogVisibility(length(uCameraPosition - vWorldPosition));
    linear_color = mix(max(uFogColor, vec3(0.0)), linear_color, fog_visibility);
...
void main()
{
    ...
    vec3 direction = normalize(
        uCameraForward + uCameraRight * (ndc.x * uAspect * uTanHalfFov) +
