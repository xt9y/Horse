...
inline std::string compatibleMainFragment()
{
    ...
    replaceOnce(
        ...
        "{\n"
        "    vec3 tangent = dFdx(vWorldPosition);\n"
        "    tangent -= normal * dot(normal, tangent);\n"
        "    if (dot(tangent, tangent) <= 1.0e-10) tangent = cross(normal, vec3(0.0, 1.0, 0.0));\n"
        ...
        "    if (strength <= 1.0e-5) return distributionGGX(n, h, roughness);\n"
        "    vec3 t = surfaceTangent(n), b = normalize(cross(n, t));\n"
        "    float c = cos(uAnisotropyRotation), s = sin(uAnisotropyRotation);\n"
        "    vec3 tr = t * c + b * s, br = -t * s + b * c;\n"
        "    float a = max(roughness * roughness, 0.0025);\n"
        "    float aspect = sqrt(max(1.0 - 0.9 * strength, 0.1));\n"
        "    float ax = max(a / aspect, 0.0025), ay = max(a * aspect, 0.0025);\n"
        "    float hx = dot(h, tr) / ax, hy = dot(h, br) / ay, hz = max(dot(h, n), 0.0);\n"
        "    float denominator = hx * hx + hy * hy + hz * hz;\n"
        "    return 1.0 / max(PI * ax * ay * denominator * denominator, 1.0e-6);\n"
        "}\n"
        "vec3 iridescentF0(vec3 base_f0, float cosine)\n"
        "{\n"
        "    float factor = clamp(uIridescence, 0.0, 1.0);\n"
        "    if (factor <= 0.0) return base_f0;\n"
        "    float thickness = mix(max(uIridescenceThicknessMin, 0.0), max(uIridescenceThicknessMax, 0.0), 0.5);\n"
        "    float phase = max(uIridescenceIor, 1.0) * thickness * max(cosine, 0.0) * 0.0125663706;\n"
        "    vec3 film = 0.5 + 0.5 * cos(vec3(phase, phase * 1.29 + 2.0944, phase * 1.61 + 4.18879));\n"
        "    return mix(base_f0, film, factor);\n"
        "}\n"
        "vec3 mappedNormal(vec3 normal)\n{\n"
    );
    replaceOnce(
    ...
    replaceOnce(
        ...
        "    float nl = max(dot(n, l), 0.0), nv = max(dot(n, v), 0.0);\n"
        "    if (nl <= 0.0 || nv <= 0.0) return vec3(0.0);\n"
        "    vec3 f0 = mix(iridescentF0(dielectricF0(), max(dot(n, v), 0.0)), albedo, metallic);\n"
        "    vec3 f = fresnelSchlick(max(dot(h, v), 0.0), f0);\n"
        "    float d = anisotropicDistribution(n, h, roughness), g = geometrySmith(n, v, l, roughness);\n"
    ...
    replaceOnce(
        ...
        "    vec3 ambient = max(uAmbientColor, vec3(0.0)) * max(uAmbientIntensity, 0.0) * albedo * ao;\n"
        "    vec3 reflected = reflect(-view_direction, normal);\n"
        "    vec3 f0 = mix(iridescentF0(dielectricF0(), max(dot(normal, view_direction), 0.0)), albedo, metallic);\n"
        "    vec3 ibl = max(uEnvironmentAverage, vec3(0.0)) * max(uEnvironmentIntensity, 0.0) *\n"
        "        albedo * (1.0 - metallic) * ao / PI +\n"
        "        environmentColor(reflected, roughness) * fresnelSchlick(max(dot(normal, view_direction), 0.0), f0) * ao;\n"
        "    if (uClearcoat > 0.0)\n"
        ...
        "        float dispersion = max(uDispersion, 0.0) * 0.01;\n"
        "        float eta_g = 1.0 / max(uIor, 1.0001);\n"
        "        vec3 refracted_g = refract(-view_direction, normal, eta_g);\n"
        "        if (dot(refracted_g, refracted_g) <= 1.0e-8) refracted_g = -view_direction;\n"
        "        vec3 transmitted = environmentColor(refracted_g, roughness);\n"
        "        if (dispersion > 0.0) {\n"
        "            vec3 refracted_r = refract(-view_direction, normal, 1.0 / max(uIor + dispersion, 1.0001));\n"
        "            vec3 refracted_b = refract(-view_direction, normal, 1.0 / max(uIor - dispersion, 1.0001));\n"
        "            if (dot(refracted_r, refracted_r) <= 1.0e-8) refracted_r = refracted_g;\n"
        "            if (dot(refracted_b, refracted_b) <= 1.0e-8) refracted_b = refracted_g;\n"
        "            transmitted = vec3(environmentColor(refracted_r, roughness).r, transmitted.g, environmentColor(refracted_b, roughness).b);\n"
        "        }\n"
