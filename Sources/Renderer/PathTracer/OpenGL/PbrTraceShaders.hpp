...
struct Node { vec3 bmin; uint first; vec3 bmax; uint meta; uvec4 extra; };
struct Triangle {
...
uniform sampler2D uTexture0; uniform sampler2D uTexture1;
uniform sampler2D uTexture2; uniform sampler2D uTexture3;
uniform sampler2D uTexture4; uniform sampler2D uTexture5;
uniform sampler2D uTexture6; uniform sampler2D uTexture7;
uniform sampler2D uTexture8; uniform sampler2D uTexture9;
uniform sampler2D uTexture10; uniform sampler2D uTexture11;
uniform sampler2D uTexture12; uniform sampler2D uTexture13;
uniform sampler2D uTexture14; uniform sampler2D uTexture15;
...
#define HORSE_LIGHT_TYPE int(gi_data[9].w + 0.5)
#define HORSE_LIGHT_POSITION gi_data[8].xyz
#define HORSE_LIGHT_INTENSITY gi_data[8].w
#define HORSE_LIGHT_DIRECTION gi_data[9].xyz
#define HORSE_LIGHT_COLOR gi_data[10].rgb
#define HORSE_LIGHT_RANGE gi_data[10].w
#define HORSE_SPOT_INNER gi_data[11].x
#define HORSE_SPOT_OUTER gi_data[11].y
...
{
    return float((uint(material.data.z) >> uint(index * 8)) & 255u) * (1.0 / 255.0);
}
...
{
    Hit best;
    best.found = false; best.distance = max_distance;
    best.position = vec3(0.0); best.normal = vec3(0.0, 1.0, 0.0);
    best.geometric_normal = vec3(0.0, 1.0, 0.0); best.uv = vec2(0.0);
    best.material = 0u; best.triangle = 0u;
    ...
    while (node_index < uint(uNodeCount) && steps++ < MAX_CLOSEST_STEPS) {
        ...
        if (aabbEntry(origin, inverse_direction, node.bmin, node.bmax, best.distance) >= INF) {
            node_index = node.extra.x; continue;
        }
        if ((node.meta & LEAF_BIT) != 0u) {
            ...
            for (uint triangle_index = node.first; triangle_index < end; ++triangle_index) {
                ...
                vec2 candidate_uv = triangle.uv01.xy * barycentric.x +
                    triangle.uv01.zw * barycentric.y + triangle.uv2.xy * barycentric.z;
                if (!alphaCutoutPass(material_index, candidate_uv)) continue;

                best.found = true; best.distance = distance;
                best.position = origin + direction * distance;
                best.normal = normalize(triangle.n0.xyz * barycentric.x +
                    triangle.n1.xyz * barycentric.y + triangle.n2.xyz * barycentric.z);
                ...
                best.uv = candidate_uv; best.material = material_index; best.triangle = triangle_index;
            }
    ...
    while (node_index < uint(uNodeCount) && steps++ < MAX_SHADOW_STEPS) {
        ...
        if ((node.meta & LEAF_BIT) != 0u) {
            ...
            for (uint triangle_index = node.first; triangle_index < end; ++triangle_index) {
                ...
                vec2 candidate_uv = triangle.uv01.xy * barycentric.x +
                    triangle.uv01.zw * barycentric.y + triangle.uv2.xy * barycentric.z;
                if (alphaCutoutPass(material_index, candidate_uv)) return true;
            }
            node_index = node.extra.x;
        } else {
            node_index = node.first;
...
{
    vec3 average = max(gi_data[3].rgb, vec3(0.0));
    float intensity = max(gi_data[3].a, 0.0);
    if (gi_data[4].a < 0.5) return average * intensity;
    vec3 sampled = pow(max(sampleTextureSlot(0, environmentUv(direction)).rgb, vec3(0.0)), vec3(2.2));
    return mix(sampled, average, clamp(roughness * roughness, 0.0, 1.0)) * intensity;
}

vec3 environmentBackground(vec3 direction)
{
    vec3 sky = max(gi_data[4].rgb, vec3(0.0));
    float intensity = max(gi_data[3].a, 0.0);
    if (gi_data[4].a < 0.5) return sky * intensity;
    return pow(max(sampleTextureSlot(0, environmentUv(direction)).rgb, vec3(0.0)), vec3(2.2)) * intensity;
}

vec3 applyEnvironmentFog(vec3 color, float distance_to_camera)
{
    int mode = int(gi_data[5].a + 0.5);
    if (mode == 0) return color;
    float visibility = 1.0;
    if (mode == 1) {
        visibility = clamp((gi_data[7].z - distance_to_camera) /
            max(gi_data[7].z - gi_data[7].y, 1.0e-5), 0.0, 1.0);
    } else if (mode == 2) {
        visibility = exp(-max(gi_data[7].x, 0.0) * distance_to_camera);
    }
    return mix(max(gi_data[5].rgb, vec3(0.0)), color, visibility);
}
...
vec3 tangentNormal(Hit hit, Material material)
{
    ...
    vec3 edge1 = triangle.p1.xyz - triangle.p0.xyz;
    vec3 edge2 = triangle.p2.xyz - triangle.p0.xyz;
    vec2 uv0 = triangle.uv01.xy, uv1 = triangle.uv01.zw, uv2 = triangle.uv2.xy;
...
{
    vec3 n = normalize(surface.normal), v = normalize(view_direction), l = normalize(light_direction);
...
vec3 sampleGlobalIllumination(vec3 position, vec3 normal)
{
    ...
    vec3 minimum = gi_data[0].xyz, maximum = gi_data[1].xyz;
    ...
    for (int coefficient = 0; coefficient < 4; ++coefficient) {
        ...
        vec3 c00 = mix(c000, c100, t.x), c10 = mix(c010, c110, t.x);
        vec3 c01 = mix(c001, c101, t.x), c11 = mix(c011, c111, t.x);
...
float localLightAttenuation(float distance_to_light)
{
    if (HORSE_LIGHT_RANGE <= 0.0) return 1.0;
    float ratio = clamp(distance_to_light / HORSE_LIGHT_RANGE, 0.0, 1.0);
    float falloff = 1.0 - ratio * ratio;
    return falloff * falloff;
}

float spotAttenuation(vec3 light_to_surface)
{
    float cosine = dot(normalize(HORSE_LIGHT_DIRECTION), normalize(light_to_surface));
    return clamp((cosine - HORSE_SPOT_OUTER) / max(HORSE_SPOT_INNER - HORSE_SPOT_OUTER, 1.0e-5), 0.0, 1.0);
}

vec3 tracePath(vec3 origin, vec3 direction)
{
    Hit hit = traceClosest(origin, direction, INF, true);
    if (!hit.found) return environmentBackground(direction);

    Surface surface = surfaceAt(hit);
    vec3 radiance = surface.emissive;
    vec3 view_direction = -direction;
    int light_type = HORSE_LIGHT_TYPE;
    float light_intensity = max(HORSE_LIGHT_INTENSITY, 0.0);

    if (light_type != 0 && light_intensity > 0.0) {
        vec3 light_direction;
        vec3 incoming = max(HORSE_LIGHT_COLOR, vec3(0.0)) * light_intensity;
        float shadow_distance = INF;
        if (light_type == 1 || light_type == 3) {
            vec3 to_light = HORSE_LIGHT_POSITION - hit.position;
            ...
            incoming *= localLightAttenuation(light_distance) / distance_squared;
            shadow_distance = max(light_distance - RAY_EPSILON * 8.0, 0.0);
            if (light_type == 3) incoming *= spotAttenuation(hit.position - HORSE_LIGHT_POSITION);
        } else {
            light_direction = normalize(-HORSE_LIGHT_DIRECTION);
        }
    ...
    radiance += surface.albedo * (1.0 - surface.metallic) * surface.ao * indirect_irradiance / PI;
    radiance += max(gi_data[6].rgb, vec3(0.0)) * max(gi_data[6].a, 0.0) * surface.albedo * surface.ao;

    vec3 f0 = mix(vec3(0.04), surface.albedo, surface.metallic);
    vec3 f = fresnelSchlick(max(dot(surface.normal, view_direction), 0.0), f0);
    vec3 reflected = reflect(-view_direction, surface.normal);
    vec3 env_specular = environmentRadiance(reflected, surface.roughness) * f * surface.ao;
    vec3 env_diffuse = max(gi_data[3].rgb, vec3(0.0)) * max(gi_data[3].a, 0.0) *
        surface.albedo * (1.0 - surface.metallic) * surface.ao / PI;
...
void main()
{
    ...
    if (uResetAccumulation != 0) imageStore(uAccumulation, pixel, vec4(0.0));

    if (uCameraMoving != 0) {
        int block = max(uMovingDepthBlock, 1);
        if ((pixel.x % block) == 0 && (pixel.y % block) == 0) {
            vec2 sample_pixel = min(vec2(pixel) + vec2(float(block) * 0.5), uResolution - vec2(0.5));
            float depth_value = deterministicDepth(sample_pixel);
            ivec2 block_end = min(pixel + ivec2(block), size);
        ...
        imageStore(uPrimaryDepth, pixel, vec4(deterministicDepth(vec2(pixel) + vec2(0.5)), 0.0, 0.0, 1.0));
    }
    ...
    int phase_x = phase % phase_grid, phase_y = phase / phase_grid;
    ...
    for (int sample = 0; sample < samples; ++sample) {
        ...
        uint state = hashUint(uint(pixel.x) * 1973u ^ uint(pixel.y) * 9277u ^
            absolute_sample * 26699u ^ uint(phase_x + phase_y * phase_grid) * 104729u ^ 0x68bc21ebu);
